...
from monitor import (
    get_channel_info, check_and_push,
    fetch_rss_videos, is_recent, format_push_message,
)
...
async def cmd_sub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new:
        ...
        videos = await loop.run_in_executor(
            None, lambda: fetch_rss_videos(info['channel_id'])
        )
        ...
        for video in videos:
            ...
            try:
                ...
                transcript_result = await loop.run_in_executor(
                    None, lambda u=video_url: get_transcript(u)
                )
                if not transcript_result['success']:
                    logger.warning(f"订阅测试推送字幕失败: {transcript_result['error']}")
                    continue
                report = await loop.run_in_executor(
                    None,
                    lambda t=title, tr=transcript_result['text']: generate_report(
                        transcript=tr,
                        title=t,
                ...
                # )
                if not report.get('success'):
                    logger.warning(f"订阅测试推送报告失败: {report.get('error')}")
                    continue

                msg_text = format_push_message(title, info['channel_name'], report, video_url)
                await context.bot.send_message(
                    chat_id=user_id,
                    text=msg_text,
                    parse_mode='HTML',
                )
                mark_video_processed(video['video_id'], info['channel_id'], title)
                pushed = True
                logger.info(f"订阅测试推送成功: {title}")
            except Exception as e:
                logger.error(f"订阅测试推送异常: {e}")
        ...
        if not pushed:
            await context.bot.send_message(
                ...
                text=(
                    f"✅ 订阅成功！\n\n"
                    f"频道：<b>{esc(info['channel_name'])}</b>\n\n"
                    f"该频道最近没有可推送的新视频（26小时内无更新）。\n"
                    f"下次定时检测时会自动推送新视频给你。\n"
                    f"检测频率：每 {CHECK_INTERVAL_HOURS} 小时一次"
                ),
                parse_mode='HTML',
            )
    else:
...
def main():
    acquire_lock() #第一个版本要正确使用考虑锁的范围 lock = acquire_lock() 
    init_db()
    print("数据库初始化完成")
